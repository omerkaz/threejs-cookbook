...
| Area | Path | Notes |
|---|---|---|
| Recipes | `src/recipes/scenes/*.ts` | 15 self-contained scene modules; each file is also the displayed source |
...
| Feedback | `src/engine/feedback.ts` | ping-pong render-target density field: decay/advect + one-draw-call splats |
...
prop definitions) plus a `create({ scene, camera, renderer, variant, props })`
factory returning `{ update?, preRender?, applyProps?, dispose? }`. Props whose
change can be applied live return `true` from `applyProps`; anything else
triggers a clean rebuild.

`preRender(dt)` is for recipes that own render targets: the harness calls it
immediately before the main render (the thumbnailer once per warm-up step),
and both callers reset the renderer afterwards, so a recipe can never leave a
bound target or `autoClear` off behind for the next one.

Copying a recipe out only requires the `three` package — the small engine utils
it imports (`rng`, `noise`, `feedback`) are appended to the copy automatically.
